...
{
    public class CollisionZone2D<T> where T : class, ICollisionObjectHolder2D<T>
    {
        public LinkedList<T> ListObjectInZoneAndOverlappingParents;

        public CollisionZone2D<T>[] ArraySubZone;

        int ZoneX;
        int ZoneY;
        int SubdivisionSize;

        public CollisionZone2D(int Size, int NumberOfSubdivisions, int ZoneX, int ZoneY)
        {
            this.ZoneX = ZoneX;
            this.ZoneY = ZoneY;

            ListObjectInZoneAndOverlappingParents = new LinkedList<T>();
            ArraySubZone = new CollisionZone2D<T>[NumberOfSubdivisions * NumberOfSubdivisions];
            if (NumberOfSubdivisions > 0)
            {
                ...
                for (int X = 0; X < NumberOfSubdivisions; ++X)
                {
                    ...
                    {
                        ArraySubZone[X + Y * NumberOfSubdivisions] = new CollisionZone2D<T>(SubdivisionSize, 0, ZoneX + X * NumberOfSubdivisions, ZoneY + Y * NumberOfSubdivisions);
                    }
                }
            }
        }
        ...
        public void Move(T ObjectToMove, Vector2 Offset)
        {
            ...
            CollisionZone2D<T> Root = ObjectToMove.Collision.ListParent.First.Value;

            foreach (CollisionZone2D<T> ActiveParent in ObjectToMove.Collision.ListParent)
            {
        ...
        {
            foreach (CollisionZone2D<T> ActiveParent in ObjectToRemove.Collision.ListParent)
            {
        ...
        public HashSet<T> GetCollidableObjects(T ActiveObject)
        {
            ...
            //Remove duplicates.
            foreach (CollisionZone2D<T> ActiveParent in ActiveObject.Collision.ListParent)
            {
