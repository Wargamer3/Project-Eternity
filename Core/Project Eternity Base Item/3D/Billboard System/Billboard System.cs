...
namespace ProjectEternity.Core
{
    public class BillboardSystem
    {
        ...
        public BillboardVertex[] ArrayParticles;
        ...
        public BillboardSystem(string TextureName, int MaxParticles, int NumberOfImages, BlendState BlendState, bool RotateTowardCamera, ContentManager Content, GraphicsDevice GraphicsDevice)
        {
            Init(TextureName, MaxParticles, NumberOfImages, BlendState, RotateTowardCamera, Content, GraphicsDevice);
        }

        public BillboardSystem(BinaryReader BR, int MaxParticles, ContentManager Content, GraphicsDevice GraphicsDevice)
        {
            bool RotateTowardCamera = BR.ReadBoolean();
            string TextureName = BR.ReadString();
            int NumberOfImages = BR.ReadInt32();
            int ParticleCount = BR.ReadInt32();
            bool RepeatX = BR.ReadBoolean();
            bool RepeatY = BR.ReadBoolean();
            bool RepeatZ = BR.ReadBoolean();
            Vector3 RepeatOffset = new Vector3(BR.ReadSingle(), BR.ReadSingle(), BR.ReadSingle());
            Vector3 Speed = new Vector3(BR.ReadSingle(), BR.ReadSingle(), BR.ReadSingle());
            Init(TextureName, MaxParticles, NumberOfImages, BlendState.NonPremultiplied, RotateTowardCamera, Content, GraphicsDevice);
            ...
            {
                Parameters["Size"].SetValue(new Vector2(BR.ReadSingle(), BR.ReadSingle()));

                for (int P = 0; P < ParticleCount; ++P)
                {
                    AddParticle(new Vector3(BR.ReadSingle(), BR.ReadSingle(), BR.ReadSingle()));
                }
            }
            else
            {
                for (int P = 0; P < ParticleCount; ++P)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                        ArrayPosition[i] = new Vector3(BR.ReadSingle(), BR.ReadSingle(), BR.ReadSingle());

                    AddParticle(ArrayPosition);
                }
            }
        }
        ...
        {
            int stride = BillboardVertex.SizeInBytes;
