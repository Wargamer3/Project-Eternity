...
namespace ProjectEternity.GameScreens.SorcererStreetScreen
{
    public class PERAttack : Projectile3D
    {
            ...
            : base(Lifetime)
        {
            ...
            if (!string.IsNullOrEmpty(PERAttributes.ProjectileAnimation.Path))
            {
                if (PERAttributes.ProjectileAnimation.IsAnimated)
                {
                    Map3DComponent = new Attack3D(GameScreen.GraphicsDevice, Map.Content.Load<Effect>("Shaders/Billboard 3D"), PERAttributes.ProjectileAnimation.ActualSprite.ActiveSprite, PERAttributes.ProjectileAnimation.ActualSprite.FrameCount);
                }
                else if (PERAttributes.ProjectileAnimation.ActualSprite != null)
                {
                    Map3DComponent = new Attack3D(GameScreen.GraphicsDevice, Map.Content.Load<Effect>("Shaders/Billboard 3D"), PERAttributes.ProjectileAnimation.ActualSprite.ActiveSprite, PERAttributes.ProjectileAnimation.ActualSprite.FrameCount);
                }
                else
                {
                    Map3DComponent = new Attack3D(GameScreen.GraphicsDevice, Map.Content.Load<Effect>("Shaders/Billboard 3D"), PERAttributes.ProjectileAnimation.StaticSprite, 1);
                }
            ...
            Unit3DModel = PERAttributes.Projectile3DModel;
        }
        ...
        public bool ProcessMovement(Vector3 NextPosition, Terrain ActiveTerrain)
        {
            ...
            if (ActiveTarget != null)
            {
                ...
                else
                {
                    ...
                    Map.AttackDirectly(PlayerIndex, new List<Vector3>(), ListAttackTarget);
                }
        ...
        public static List<MovementAlgorithmTile> PredictAttackMovement(SorcererStreetMap Map, Vector3 StartPosition, Vector3 CursorPosition)
        {
