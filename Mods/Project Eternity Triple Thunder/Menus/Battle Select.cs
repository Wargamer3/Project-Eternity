...
namespace ProjectEternity.GameScreens.TripleThunderScreen
{
    public class BattleSelect : GameScreen, IMissionSelect
    {
        ...
        public void AddPlayer(Player NewPlayer)
        {
            ...
            NewPlayer.CharacterPreview = new RobotAnimation("Players/" + NewPlayer.Equipment.CharacterType, null, Vector2.Zero, 0, new PlayerEquipment(), new MuteSFXGenerator(), new List<Weapon>());
        ...
        public void UpdateCharacter(Player PlayerToUpdate)
        {
            ...
            PlayerToUpdate.CharacterPreview = new RobotAnimation("Players/" + PlayerToUpdate.Equipment.CharacterType, null, Vector2.Zero, 0, new PlayerEquipment(), new MuteSFXGenerator(), new List<Weapon>());
        }
        ...
        private void OpenModeSelectScreen()
        {
            if (OnlineClient != null)
            {
                if (IsHost)
                {
                    PushScreen(new BattleModeSelect(Room, this, OnlineClient));
                    sndButtonClick.Play();
                }
            }
            else
        ...
        private void KillMinus()
        {
            if (OnlineClient != null)
            {
                if (IsHost)
                {
                    OnlineClient.Host.Send(new AskChangeRoomExtrasBattleScriptClient(Room.MaxKill - 1, Room.MaxGameLengthInMinutes));
                }
            }
            else
            {
        ...
        private void TimeMinus()
        {
            if (OnlineClient != null)
            {
                if (IsHost)
                {
                    OnlineClient.Host.Send(new AskChangeRoomExtrasBattleScriptClient(Room.MaxKill, Room.MaxGameLengthInMinutes - 1));
                }
            }
            else
            {
        ...
        private void RedTeamSelected()
        {
            ...
            if (OnlineClient != null)
            {
                OnlineClient.Host.Send(new AskChangeTeamScriptClient(0));
            }
            else
            {
                LocalPlayer.Team = 0;
        ...
        private void BlueTeamSelected()
        {
            ...
            if (OnlineClient != null)
            {
                OnlineClient.Host.Send(new AskChangeTeamScriptClient(1));
            }
            else
            {
                LocalPlayer.Team = 0;
