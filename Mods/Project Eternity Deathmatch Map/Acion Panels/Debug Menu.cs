...
namespace ProjectEternity.GameScreens.DeathmatchMapScreen
{
    ...
    {
        private const string PanelName = "Info";

        Squad ActiveSquad;
        private List<List<BaseEffect>> ListSquadEffect;

        public ActionPanelDebug(DeathmatchMap Map)
            : base(PanelName, Map)
        {
        }
        ...
        {
            return new ActionPanelDebug(Map);
        }
        ...
        public override void Draw(CustomSpriteBatch g)
        {
            for (int U = 0; U < ActiveSquad.UnitsAliveInSquad; ++U)
            {
                ...
                TextHelper.DrawText(g, ActiveSquad[U].RelativePath, new Vector2(X + 5, 25), Color.White);
                for (int E = 0; E < ListSquadEffect[U].Count; ++E)
                {
                    TextHelper.DrawText(g, ListSquadEffect[U][E].ToString(), new Vector2(X + 10, 45 + E * 20), Color.White);
                }
            }
        }
    }
