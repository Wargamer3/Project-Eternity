...
namespace ProjectEternity.Editors.SorcererStreetMapEditor
{
    class SorcererStreetTilesetTab : MapEditor.TilesetTab
    {
        protected override void btnTileAttributes_Click(object sender, EventArgs e)
        {
            ...
            SorcererStreetTileAttributes TA = new SorcererStreetTileAttributes(SelectedTerrain.TerrainTypeIndex, SelectedTerrain.Height);
            if (TA.ShowDialog() == DialogResult.OK)
            {
                ActiveMap.ListTilesetPreset[cboTiles.SelectedIndex].ArrayTilesetInformation[0].ArrayTerrain[TilePos.X / ActiveMap.TileSize.X, TilePos.Y / ActiveMap.TileSize.Y].TerrainTypeIndex = TA.TerrainTypeIndex;
                ActiveMap.ListTilesetPreset[cboTiles.SelectedIndex].ArrayTilesetInformation[0].ArrayTerrain[TilePos.X / ActiveMap.TileSize.X, TilePos.Y / ActiveMap.TileSize.Y].Height = TA.TerrainHeight;
            }
        }
    }
}
